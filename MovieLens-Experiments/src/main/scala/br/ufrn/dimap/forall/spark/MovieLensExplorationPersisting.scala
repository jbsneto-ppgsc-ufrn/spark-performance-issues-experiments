...
object MovieLensExplorationPersisting {
  ...
  def main(args: Array[String]) {
    ...
    var persistenceLevel = StorageLevel.MEMORY_ONLY
    ...
    if (args.length > 4) {
      ...
      outputURL = args(3)
      persistenceLevel = StorageLevel.fromString(args(4))
    } else {
      println("Invalid arguments")
    }

    val conf = new SparkConf()
    conf.setAppName("MovieLens-Dataset-Exploration-Persisting-with-" + (if (args.length > 4) args(4) else "MEMORY_ONLY"))
    val sparkContext = new SparkContext(conf)

    val ratings = sparkContext.textFile(inputMLLatestRatingsURL).flatMap(parseRatings).persist(persistenceLevel)

    val movies = sparkContext.textFile(inputMLLatestMoviesURL).flatMap(parseMovies).persist(persistenceLevel)

    val tags = sparkContext.textFile(inputMLLatestTagsURL).flatMap(parseTags)

    println("How many movies were produced per year?")

    val moviesPerYear = movies.map(m => (m.year, 1)).reduceByKey(_ + _).map(m => (m._2, m._1)).sortByKey(false).persist(persistenceLevel)
    ...
    val popularGenresByYear = movies.flatMap(m => m.genres.map(g => ((m.year, g), 1))).reduceByKey(_ + _).map(g => (g._1._1, (g._1._2, g._2))).reduceByKey((m1, m2) => if (m1._2 > m2._2) m1 else m2).persist(persistenceLevel)
    ...
    val ratingsStatistics = groupByMoviesInfo.groupByKey().mapValues(x => (x.size, x.sum / x.size, x.min, x.max)).persist(persistenceLevel)
    ...
    val bestMoviesDecade = groupByDecade.mapValues(s => s.reduce((s1, s2) => if (s1._2._5 > s2._2._5) s1 else s2)).sortByKey(true).persist(persistenceLevel)
    ...
    val genresTagsCountSorted = genresTagsCount.map(g => (g._1._1, (g._1._2, g._2))).sortByKey(true).persist(persistenceLevel)
