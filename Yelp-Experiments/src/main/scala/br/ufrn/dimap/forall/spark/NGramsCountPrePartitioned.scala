...
object NGramsCountPrePartitioned {
  ...
  def main(args: Array[String]) {
    ...
    conf.setAppName("NGrams-Count-reduceByKey-pre-partitioned-Version-with-" + n.toString)
    ...
    val ngramsCount = ngramsFiltered.map(x => (x, 1)).partitionBy(new HashPartitioner(48)).reduceByKey((x, y) => x + y)
