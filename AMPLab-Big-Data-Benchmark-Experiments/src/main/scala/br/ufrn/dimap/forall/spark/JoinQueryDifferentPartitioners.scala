 ...
 */
object JoinQueryDifferentPartitioners {
  ...
  def main(args: Array[String]) {
    ...
    val subqueryR = rankings.map(r => (r.pageURL, r)).partitionBy(new OtherStringPartitioner(42))
  ...
  class OtherStringPartitioner(np: Int) extends Partitioner {

    override def numPartitions: Int = np

    override def getPartition(key: Any): Int = {
      val k = key.asInstanceOf[String]
      val a = numPartitions / 2
      var p = (k.hashCode() + a) % numPartitions // a different logic to differentiate OtherStringPartitioner from StringPartitioner 
      Math.abs(p) // make it non-negative
    }

    override def equals(other: Any): Boolean = {
      other match {
        case obj: OtherStringPartitioner => obj.numPartitions == numPartitions
        case _                           => false
      }
    }
  }
